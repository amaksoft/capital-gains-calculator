...
class TestReverseSplitParsing:
    ...
    def test_reverse_split_rows_are_combined(self, tmp_path: Path) -> None:
        ...
        csv_file.write_text(
            ...
            "12/09/2025,Reverse Split,UL,UNILEVER PLC,,164,,\n"
            "12/09/2025,Reverse Split,UL,UNILEVER PLC,,-185.0291,,\n"
        )
        ...
        assert transactions[0].symbol == "UL"
        assert transactions[0].quantity == Decimal("-21.0291")
    ...
    def test_reverse_splits_of_different_symbols_stay_separate(
        ...
        csv_file.write_text(
            ...
            "12/09/2025,Reverse Split,UL,UNILEVER PLC,,164,,\n"
            "12/09/2025,Reverse Split,UL,UNILEVER PLC,,-185.0291,,\n"
            "12/09/2025,Reverse Split,AAA,A CORP,,10,,\n"
            "12/09/2025,Reverse Split,AAA,A CORP,,-50,,\n"
        )

        transactions = SchwabParser().load_from_file(csv_file)

        assert len(transactions) == 2
        assert {txn.symbol for txn in transactions} == {"UL", "AAA"}
    ...
    def test_reverse_split_legs_on_different_dates_are_rejected(
        ...
        csv_file.write_text(
            "Date,Action,Symbol,Description,Price,Quantity,Fees & Comm,Amount\n"
            "12/10/2025,Reverse Split,UL,UNILEVER PLC,,164,,\n"
            "12/09/2025,Reverse Split,UL,UNILEVER PLC,,-185.0291,,\n"
        )
    ...
    def test_reverse_split_without_quantity_is_rejected(self, tmp_path: Path) -> None:
        ...
        csv_file.write_text(
            "Date,Action,Symbol,Description,Price,Quantity,Fees & Comm,Amount\n"
            "12/09/2025,Reverse Split,UL,UNILEVER PLC,,,,\n"
        )
    ...
    def test_interleaved_reverse_splits_are_grouped_by_symbol(
        ...
        csv_file.write_text(
            ...
            "12/09/2025,Reverse Split,AAA,A CORP,,-50,,\n"
            "12/09/2025,Reverse Split,UL,UNILEVER PLC,,-185.0291,,\n"
            "12/09/2025,Reverse Split,AAA,A CORP,,40,,\n"
        )
